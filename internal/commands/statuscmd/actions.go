...
func runStatus(ctx context.Context, cmd *cli.Command) error {
	...
	gather := func() error {
		...
		if len(sweep.statuses) == 0 && sweep.answered == 0 && len(sweep.errs) > 0 {
			if cerr := runner.NoRegionAnswered(ctx, awsCfg, sweep.skipped, sweep.regionFailures, sweep.errs); cerr != nil {
				return cerr
			}
			return sweep.errs[0]
		}
	...
	if err := allRegionsSkipped(len(regions), sweep.skipped); err != nil {
		if cerr := runner.NoRegionAnswered(ctx, awsCfg, sweep.skipped, nil, nil); cerr != nil {
			return cerr
		}
		return err
	}
