...
Exit codes: 0 ok; 1 error, interrupt, or a failed single-add-on update; 3 EKS is already changing the cluster, nothing changed; 4 with --all, an add-on update failed or was not attempted, or an add-on could not be read after its update; 5 updated, but the post-update health check found issues. See https://drod.dev/refresh/concepts/exit-codes/
